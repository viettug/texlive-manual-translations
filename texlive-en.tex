...
(manuals, man pages, Info files) is in \dirname{texmf-dist/doc}.  The
...
\dirname{TEXDIR} is \dirname{/usr/local/texlive/2013} on Unix
and |%SystemDrive%\texlive\2013| on Windows.
...
\file{.../2013/texmf.cnf}.  If you must make changes to the defaults
(not normally necessary), this is the place to put them.  The main
configuration file is in \file{.../2013/texmf-dist/web2c/texmf.cnf}.
...
\pkgname{powerpc-aix} removed.  Support for some additional platforms is
available as custom binaries
(\url{http://tug.org/texlive/custom-bin.html}).  In addition, some
platforms are omitted from the \DVD\ (simply to save space), but can be
installed normally over the net.
