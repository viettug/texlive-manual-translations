...
\begin{center}
In memory of Sebastian Rahtz, Peter Breitenlohner, and all our departed
colleagues.
\end{center}
...
pdf\TeX: Use environment variable \code{SOURCE\_DATE\_EPOCH} (if set) for
timestamps; new primitives \cs{pdfinfoomitdate}, \cs{pdftrailer},
\cs{pdftrailerid}, \cs{pdfsuppressptexinfo}, to control values appearing
in the output which normally change with each run.  These features are
for PDF output only, not DVI.
